//! Benchmarks for `durable write` latency
//! Run using: `taskset -c 2,3,4 cargo bench --bench durable_write`
...
fn single_tx_write_latency() -> BenchResult {
    ...
    for i in 0..WARMUP_OPS {
        let ticket = unsafe { rta.write(|t| t.0 = [i as u64; 4]) }.expect("warmup write");
        ticket.wait().unwrap();
    }
...
fn print_results(single: &BenchResult, multi: &BenchResult) {
    ...
    println!();
    println!("| Metric  | Single TX (ms) | Multi TX (ms) |");
    println!("|:--------|:---------------|:--------------|");
    println!(
        "| P50     | {:>14.4} | {:>13.4} |",
        single.hist.value_at_quantile(0.50) as f64 / 1_000_000.0,
        multi.hist.value_at_quantile(0.50) as f64 / 1_000_000.0,
    );
    println!(
        "| P90     | {:>14.4} | {:>13.4} |",
        single.hist.value_at_quantile(0.90) as f64 / 1_000_000.0,
        multi.hist.value_at_quantile(0.90) as f64 / 1_000_000.0,
    );
    println!(
        "| P99     | {:>14.4} | {:>13.4} |",
        single.hist.value_at_quantile(0.99) as f64 / 1_000_000.0,
        multi.hist.value_at_quantile(0.99) as f64 / 1_000_000.0,
    );
    println!(
        "| MEAN    | {:>14.4} | {:>13.4} |",
        single.hist.mean() as f64 / 1_000_000.0,
        multi.hist.mean() as f64 / 1_000_000.0,
    );
    println!(
        "| MAX     | {:>14.4} | {:>13.4} |",
        single.hist.max() as f64 / 1_000_000.0,
        multi.hist.max() as f64 / 1_000_000.0,
    );
    println!();
}
